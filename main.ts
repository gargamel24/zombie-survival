...
info.onCountdownEnd(function () {
    ...
    game.showLongText("Wave " + level + " is coming", DialogLayout.Bottom)
    nextLevel()
})
function nextLevel () {
    if (level == 1) {
...
sprites.onOverlap(SpriteKind.Player, SpriteKind.Enemy, function (sprite, otherSprite) {
    game.splash("Oh no! You are bitten.", "You are going to turn into a zombie!")
    game.over(false)
})
let sayingNumber = 0
...
let zombieImg: Image = null
let z: Sprite = null
let cloneImg: Image = null
let location = null
zombieImgs = [
img`
    ...cccccccccccccc...
...
let zombieSays = ["Grrrrr", "Eat your Brains", "Booooo", "Arrrrr"]
scene.setBackgroundColor(7)
tiles.setTilemap(tilemap`level_1`)
level = 1
nextLevel()
// Change the zombie to face the other direction depending
// where the old lady is
game.onUpdate(function () {
    for (let z2 of sprites.allOfKind(SpriteKind.Enemy)) {
        if (z2.vx < 0) {
            z2.setImage(sprites.readDataImage(z2, "costume-left"))
        } else {
            z2.setImage(sprites.readDataImage(z2, "costume-right"))
        }
    }
})
game.onUpdateInterval(1000, function () {
    ...
    // Pick a random thing for the zombie to say
    sayingNumber = randint(0, zombieSays.length - 1)
    zombies[randomZombieNumber].say(zombieSays[sayingNumber], 3000)
})
